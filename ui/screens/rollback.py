...
class RollbackScreen(DBLMScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="rollback-root"):
            ...
            yield Static("Inspect revertable actions, recorded runs, and backup-based recovery options.", id="rollback-subtitle")
    ...
    def _build_actions_text(self, latest_run) -> str:
        ...
        lines = [f"- {a.target} -> {a.subvolume or 'n/a'} [status={a.status}, backup={'yes' if a.backup_id else 'no'}]" for a in latest_run.actions]
        return "[bold]Revertable actions[/bold]\n\n" + "\n".join(lines)

    def _build_backups_text(self, restorable_backups) -> str:
        if not restorable_backups:
            return "[bold]Backup recovery[/bold]\n\nNo restorable backups are currently available."
        lines = [f"- {b.backup_id}: {b.original_path} <- {b.backup_path}" for b in restorable_backups[:10]]
        return (
    ...
    def _build_subvolumes_text(self, latest_run) -> str:
        if latest_run is None or not latest_run.actions:
            return "[bold]Subvolume removal[/bold]\n\nNo created subvolumes are currently recorded."
        lines = [f"- target={a.target} subvolume={a.subvolume} scope={a.filesystem_scope}" for a in latest_run.actions if a.subvolume]
        if not lines:
