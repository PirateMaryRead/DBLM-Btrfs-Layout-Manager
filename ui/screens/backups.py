    ...
    def _build_available_text(self, backups) -> str:
        ...
        for backup in backups[:10]:
            lines.append(
                ...
                f"  exists on disk: {yes_no(backup.exists_on_disk())}\n"
                f"  kind: {backup.kind}"
            )
    ...
    def _build_restorable_text(self, backups) -> str:
        ...
        for backup in backups[:10]:
            lines.append(
                ...
                f"  restored before: {yes_no(bool(backup.restored_at))}"
            )
    ...
    def _build_deleted_text(self, backups) -> str:
        ...
        for backup in backups[:10]:
            lines.append(
                f"- {backup.backup_id}\n"
                f"  original: {backup.original_path}\n"
                f"  deleted at: {safe_text(backup.deleted_at)}\n"
                f"  still on disk: {yes_no(Path(backup.backup_path).exists())}"
            )
